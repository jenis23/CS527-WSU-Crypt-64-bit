...
public class Project1 {
	...
	private static final String CIPHER_TEXT_FILE = "cyphertext.txt";
	private static final String DECIPHER_TEXT_FILE = "decyphertext.txt";
	/**
	 * @param args
	 */
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < PLAIN_TEXT.length; i++) {
			...
			cipherContents.write(CIPHER_TEXT[i]);
			DECIPHER_TEXT[i] = WSU_DECRYPTION(CIPHER_TEXT[i], KEY_TEXT);
			decipherContents.write(DECIPHER_TEXT[i]);
		}
		cipherContents.close();
		decipherContents.close();
		/*
		for (int i = 0; i < PLAIN_TEXT.length; i++) {
			System.out.print("Cipher Text[" + i + "]:" + CIPHER_TEXT[i]);
		}
		for (int i = 0; i < PLAIN_TEXT.length; i++) {
			System.out.print("Decipher Text[" + i + "]:" + DECIPHER_TEXT[i]);
		}
		*/
		}catch(Exception e){
			e.printStackTrace();
	...
	public static String WSU_ENCRYPTION(String PLAIN_TEXT, String KEY_TEXT) {
		...
		for (int k = 0; k < 64; k++) {
			if (binaryKeyTxt.length() < 64) {
				binaryKeyTxt = binaryKeyTxt.concat("0");
			}
		}
		...
		String r0 = stringToHex(R[0]);
		String r1 = stringToHex(R[1]);
		String r2 = stringToHex(R[2]);
		String r3 = stringToHex(R[3]);
		...
		String hex_Final1 = stringToHex(Cipher_Text[0]);
		String hex_Final2 = stringToHex(Cipher_Text[1]);
		String hex_Final3 = stringToHex(Cipher_Text[2]);
		String hex_Final4 = stringToHex(Cipher_Text[3]);

		return (hex_Final1.concat(hex_Final2).concat(hex_Final3)
				.concat(hex_Final4));
	...
	public static String WSU_DECRYPTION(String CIPHER_TEXT, String KEY_TEXT) {
		...
		String binaryCprTxt = WSUCryptHelper.HexToBinary(CIPHER_TEXT);
		for (int k = 0; k < 64; k++) {
			if (binaryCprTxt.length() != 64) {
				binaryCprTxt = binaryCprTxt.concat("0");
			}
		}
		...
		String[] Ascii_Value = new String[8];
		Ascii_Value[0] = Character.toString((char) Integer.parseInt(
				Decipher_Text[0].substring(0, 8), 2));
		Ascii_Value[1] = Character.toString((char) Integer.parseInt(
				Decipher_Text[0].substring(8, 16), 2));
		Ascii_Value[2] = Character.toString((char) Integer.parseInt(
				Decipher_Text[1].substring(0, 8), 2));
		Ascii_Value[3] = Character.toString((char) Integer.parseInt(
				Decipher_Text[1].substring(8, 16), 2));
		Ascii_Value[4] = Character.toString((char) Integer.parseInt(
				Decipher_Text[2].substring(0, 8), 2));
		Ascii_Value[5] = Character.toString((char) Integer.parseInt(
				Decipher_Text[2].substring(8, 16), 2));
		Ascii_Value[6] = Character.toString((char) Integer.parseInt(
				Decipher_Text[3].substring(0, 8), 2));
		Ascii_Value[7] = Character.toString((char) Integer.parseInt(
				Decipher_Text[3].substring(8, 16), 2));

		return Ascii_Value[0].concat(Ascii_Value[1]).concat(Ascii_Value[2])
				.concat(Ascii_Value[3]).concat(Ascii_Value[4])
				.concat(Ascii_Value[5]).concat(Ascii_Value[6])
				.concat(Ascii_Value[7]);
	...
	public static String[] F_Function(String R0, String R1, int round,
		...
		String t0 = stringToHex(T[0]);
		String t1 = stringToHex(T[1]);
